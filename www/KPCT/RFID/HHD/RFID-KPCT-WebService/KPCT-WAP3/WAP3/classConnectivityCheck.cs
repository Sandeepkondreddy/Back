...
namespace WAP3
{
    class classConnectivityCheck
    {
        ...
        public static string WebServiceConnectivityCheck()
        {
            string ConnectionStatus = "";
            try
            {
                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(classServerDetails.WebLinkWifi.ToString());
                if (classLogin.Connectivity == "GPRS")
                    request = (HttpWebRequest)WebRequest.Create(classServerDetails.WebLinkGPRS.ToString());
                classLog.writeLog("Message @: Connectivity Check: "+request.ToString());
                request.Method = "GET";
